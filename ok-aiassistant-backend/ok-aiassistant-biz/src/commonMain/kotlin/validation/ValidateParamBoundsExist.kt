...
fun ICorChainDsl<AppContext>.validateParamBoundsExist(title: String) = worker {
    this.title = title
    on {
        modelValidating.modelParams.any { param ->
            val bounds = param.bounds
            bounds.min.isNaN() || bounds.max.isNaN()
        }
    }
    handle {
        fail(
            errorValidation(
                field = "bounds",
                violationCode = "missingBounds",
                description = "Bounds must be valid numbers"
            )
        )
    }
}
